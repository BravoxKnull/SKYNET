...
function initializeFriendSystem() {
  ...
  `;
  headerRight.insertBefore(notificationIcon, headerRight.firstChild);
  
  // Add menu button to user items
...
function handleDocumentClick(e) {
  ...
  if (!notificationDropdown.contains(e.target) && !e.target.closest('.notification-icon')) {
    notificationDropdown.classList.add('hidden');
  }
}

// Add user menu to user items
function addUserMenu(userItem, userId) {
  ...
  menuButton.addEventListener('click', (e) => {
    ...
    // Create new menu
    const menuTemplate = document.getElementById('userMenuTemplate');
    const menu = menuTemplate.content.cloneNode(true);
    const menuElement = menu.querySelector('.user-menu');
    
    // Position menu
    const rect = userItem.getBoundingClientRect();
    menuElement.style.position = 'absolute';
    menuElement.style.top = `${rect.bottom + window.scrollY}px`;
    menuElement.style.left = `${rect.right + window.scrollX - 180}px`;
    
    // Add menu to DOM
    document.body.appendChild(menuElement);
    
    // Add event listeners
    menuElement.querySelector('.send-message').addEventListener('click', () => {
      openChat(userId);
      menuElement.remove();
    });
    
    menuElement.querySelector('.add-friend').addEventListener('click', () => {
      sendFriendRequest(userId);
      menuElement.remove();
    });
  });
  
  // Add menu button to user item
  const userDetails = userItem.querySelector('.user-details');
  if (userDetails) {
    userDetails.appendChild(menuButton);
  }
}
...
async function sendFriendRequest(friendId) {
  ...
  try {
    ...
    showToast('Failed to send friend request', true);
  }
}
...
function handleFriendRequest(data) {
  ...
  // Get sender info from users list
  const sender = users.find(u => u.id === data.senderId);
  if (!sender) return;
  
  // Show notification toast
  showFriendRequestToast(sender);
...
function showFriendRequestToast(sender) {
  const toastTemplate = document.getElementById('notificationToastTemplate');
  const toast = toastTemplate.content.cloneNode(true);
  const toastElement = toast.querySelector('.notification-toast');
  
  // Set sender name
  toastElement.querySelector('.notification-text').textContent = 
    `${sender.displayName} wants to be your friend`;
  
  // Add toast to DOM
  document.body.appendChild(toastElement);
  
  // Add event listeners
  toastElement.querySelector('.accept-btn').addEventListener('click', () => {
    updateFriendRequest(sender.id, 'accepted');
    toastElement.classList.add('fade-out');
    setTimeout(() => toastElement.remove(), 300);
  });
  
  toastElement.querySelector('.reject-btn').addEventListener('click', () => {
    updateFriendRequest(sender.id, 'rejected');
    toastElement.classList.add('fade-out');
    setTimeout(() => toastElement.remove(), 300);
  });
...
async function updateFriendRequest(friendId, status) {
  try {
    const { data, error } = await supabase
      .from('friends')
    ...
    showToast('Failed to update friend request', true);
  }
}
...
function handleFriendRequestUpdate(data) {
  ...
  showToast(`Friend request ${data.status}`);
  loadNotifications();
}
...
function renderNotifications() {
  ...
  notifications.forEach(notification => {
    ...
    notificationItem.innerHTML = `
      ...
      ${notification.status === 'pending' ? `
        <div class="notification-actions">
          ...
          <button class="reject-btn" data-id="${notification.id}">Reject</button>
        </div>
      ` : ''}
    `;
    ...
    // Add event listeners for pending requests
    if (notification.status === 'pending') {
      notificationItem.querySelector('.accept-btn').addEventListener('click', () => {
        updateFriendRequest(notification.user.id, 'accepted');
      });
      
      notificationItem.querySelector('.reject-btn').addEventListener('click', () => {
        updateFriendRequest(notification.user.id, 'rejected');
      });
    }
  });
}
...
function initializeChatSidebar() {
  ...
  const chatTemplate = document.getElementById('chatSidebarTemplate');
  const chatSidebar = chatTemplate.content.cloneNode(true);
...
async function openChat(friendId) {
  const friend = users.find(u => u.id === friendId) || 
    notifications.find(n => n.user.id === friendId)?.user;
...
function handleNewMessage(data) {
  if (data.senderId !== activeChat && data.receiverId !== currentUser.id) return;
  
  // If chat is open with this user, add the message
  if (data.senderId === activeChat) {
    ...
    const sender = users.find(u => u.id === data.senderId);
    if (sender) {
      showToast(`New message from ${sender.displayName}`);
    }
  }
}

// Initialize the friend system when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
  if (!initializeUserData()) {
    return;
  }
  initializeDOMElements();
  initializeEventListeners();
  initializeFriendSystem(); // Add this line
});